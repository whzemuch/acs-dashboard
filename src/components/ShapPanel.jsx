...
export default function ShapPanel() {
  ...
  const stateCode = useMemo(() => (arc?.dest ? arc.dest.slice(0, 2) : null), [arc]);
  ...
  const topStats = useMemo(() => {
    ...
    rows.sort((a, b) => (absSort ? Math.abs(b.value) - Math.abs(a.value) : b.value - a.value));
    const sliced = rows.slice(0, Math.max(1, Math.min(100, Number(topK) || 10)));
  ...
  return (
    ...
    >
      ...
      {!arc ? (
        <div style={{ fontSize: 13, color: "#6b7280" }}>Click an arc to view SHAP details.</div>
      ) : loading ? (
        <div style={{ fontSize: 13, color: "#6b7280" }}>Loading SHAP…</div>
      ) : error ? (
        <div style={{ color: "#b91c1c", fontSize: 13 }}>Failed to load SHAP. {error}</div>
      ) : entry ? (
        <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
          <div style={{ minWidth: 220 }}>
            ...
            <div style={{ fontSize: 12, color: "#4b5563" }}>
              Predicted: {arc.predicted?.toLocaleString?.(undefined, { maximumFractionDigits: 1 })}
            </div>
            <div style={{ fontSize: 12, color: "#4b5563" }}>
            ...
            <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <button type="button" onClick={() => copyShapCsv(entry, schema)} style={{ fontSize: 12 }}>
                Copy SHAP as CSV
              </button>
            </div>
          </div>
        ...
        <div style={{ fontSize: 13, color: "#6b7280" }}>No SHAP entry found for this arc.</div>
      )}
    </div>
  );
}
...
function shapToRows(values, schema) {
  const names = Array.isArray(schema) && schema.length ? schema : [];
  return (values || []).map((v, i) => ({ label: prettifyKey(names[i] || `shap_${i}`), value: Number(v) || 0 }));
}
...
async function copyShapCsv(entry, schema) {
  const rows = shapToRows(entry.shapValues, schema);
  const lines = ["feature,value", ...rows.map((r) => `${escapeCsv(r.label)},${r.value}`)];
  const csv = lines.join("\n");
  try {
    await navigator.clipboard.writeText(csv);
    // Optionally, show a toast; for now, silent success
  } catch {
